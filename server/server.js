...
function normalizeRole(r){const s=String(r||'').trim().toLowerCase();if(s==='teacher'||s==='staff')return 'Staff';if(s==='admin'||s==='administrator')return 'Admin';if(s==='student')return 'Student';if(s==='parent')return 'Parent';if(s==='activity supervisor'||s==='activitysupervisor'||s==='supervisor')return 'Activity Supervisor';return String(r||'Staff')}
...
async function sanitizeStateFor(user){
 ...
 const questions=await getSnapshot('rcs_questions',[]),comments=await getSnapshot('rcs_parent_comments',[]),results=await getSnapshot('rcs_results',[]),studentReports=await getSnapshot('rcs_student_reports',[]),teacherChat=await getSnapshot('rcs_admin_teacher_chat',[]),parentChat=await getSnapshot('rcs_admin_parent_chat',[]);
 const allAtt=await getAllAttendance();
 if(user.role==='Admin') return {rcs_people:(Array.isArray(people)?people:[]).map(stripPerson),rcs_users:(Array.isArray(allUsers)?allUsers:[]).map(safeUser),rcs_greports:await getSnapshot('rcs_greports',[]),rcs_assignments:assignments,rcs_answers:answers,rcs_att:allAtt,resurgence_daily_reports:daily,rcs_student_reports:studentReports,rcs_class_attendance:await getSnapshot('rcs_class_attendance',[]),rcs_admin_teacher_chat:teacherChat,rcs_admin_parent_chat:parentChat};
 ...
 if(user.role==='Student'){
   ...
   return {rcs_people:[...mine,...classStudents],rcs_users:(Array.isArray(allUsers)?allUsers:[]).filter(u=>u&&String(u.uid||'')===uid).map(safeUser),rcs_assignments:visible,rcs_answers:(Array.isArray(answers)?answers:[]).filter(a=>String(a.studentUid||'')===uid),rcs_greports:[],rcs_att:myAtt,resurgence_daily_reports:myDaily,rcs_questions:(Array.isArray(questions)?questions:[]).filter(q=>String(q.studentUid||'')===uid),rcs_parent_comments:[],rcs_results:[],rcs_student_reports:(Array.isArray(studentReports)?studentReports:[]).filter(r=>String(r.studentUid||'')===uid),rcs_admin_teacher_chat:[],rcs_admin_parent_chat:[]};
 }
 if(user.role==='Activity Supervisor'){const students=(Array.isArray(people)?people:[]).filter(p=>normalizeRole(p.role)==='Student').map(stripPerson);return {rcs_people:students,rcs_users:[],rcs_assignments:[],rcs_answers:[],rcs_greports:[],rcs_att:allAtt,resurgence_daily_reports:[],rcs_questions:[],rcs_parent_comments:[],rcs_results:[],rcs_student_reports:[],rcs_class_attendance:await getSnapshot('rcs_class_attendance',[]),rcs_admin_teacher_chat:[],rcs_admin_parent_chat:[]};}
 const staffClass=String(mine[0]?.className||''); const classStudents=(Array.isArray(people)?people:[]).filter(p=>normalizeRole(p.role)==='Student'&&String(p.className||'')===staffClass).map(stripPerson); const staffQuestions=(Array.isArray(questions)?questions:[]).filter(q=>String(q.staffUid||'')===uid||String(q.className||'')===staffClass); const staffComments=(Array.isArray(comments)?comments:[]).filter(c=>String(c.staffUid||'')===uid||String(c.staffClass||'')===staffClass); const staffResults=(Array.isArray(results)?results:[]).filter(r=>String(r.staffUid||'')===uid); const staffStudentReports=(Array.isArray(studentReports)?studentReports:[]).filter(r=>String(r.staffUid||'')===uid||String(r.className||'')===staffClass); return {rcs_people:mine,rcs_users:(Array.isArray(allUsers)?allUsers:[]).filter(u=>u&&String(u.uid||'')===uid).map(safeUser),rcs_assignments:Array.isArray(assignments)?assignments:[],rcs_answers:Array.isArray(answers)?answers:[],rcs_greports:[],rcs_att:myAtt,resurgence_daily_reports:myDaily,rcs_questions:staffQuestions,rcs_parent_comments:staffComments,rcs_results:staffResults,rcs_student_reports:staffStudentReports,rcs_class_attendance:await getSnapshot('rcs_class_attendance',[]),rcs_admin_teacher_chat:teacherChat,rcs_admin_parent_chat:[]};
}
...
async function syncAuthFromUsers(users){
 ...
 try{
  ...
  for(const raw of rows){if(!raw)continue;
   ...
   const role=normalizeRole(raw.role); if(!['Admin','Staff','Student','Parent','Activity Supervisor'].includes(role))continue;
...
app.post('/api/chat/:type',auth,async(req,res)=>{try{const type=String(req.params.type||'');const map={teacher:{key:'rcs_admin_teacher_chat',roles:['Admin','Staff']},parent:{key:'rcs_admin_parent_chat',roles:['Admin','Parent']}};const cfg=map[type];if(!cfg||!cfg.roles.includes(req.user.role))return res.status(403).json({error:'Chat access denied'});const text=String(req.body?.message||'').trim();if(!text)return res.status(400).json({error:'Message cannot be empty'});if(text.length>5000)return res.status(400).json({error:'Message is too long'});const arr=await getSnapshot(cfg.key,[]);arr.push({id:'chat-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,8),fromUid:req.user.uid||'',fromName:req.user.name||req.user.username||'User',fromRole:req.user.role,text,createdAt:new Date().toISOString(),readBy:{[String(req.user.uid||'')]:true}});await saveSnapshot(cfg.key,arr);return res.json({ok:true,state:await sanitizeStateFor(req.user)})}catch(e){console.error(e);res.status(500).json({error:'Unable to send chat message'})}});
app.post('/api/chat/:type/read',auth,async(req,res)=>{try{const type=String(req.params.type||'');const map={teacher:{key:'rcs_admin_teacher_chat',roles:['Admin','Staff']},parent:{key:'rcs_admin_parent_chat',roles:['Admin','Parent']}};const cfg=map[type];if(!cfg||!cfg.roles.includes(req.user.role))return res.status(403).json({error:'Chat access denied'});const uid=String(req.user.uid||'');const arr=await getSnapshot(cfg.key,[]);await saveSnapshot(cfg.key,arr.map(m=>({...m,readBy:{...(m.readBy||{}),[uid]:true}})));return res.json({ok:true,state:await sanitizeStateFor(req.user)})}catch(e){console.error(e);res.status(500).json({error:'Unable to update chat notifications'})}});
app.post('/api/snapshot/:key',auth,async(req,res)=>{try{const key=req.params.key,value=Array.isArray(req.body.value)?req.body.value:[];
 ...
 if(key==='rcs_results'){if(!['Admin','Staff'].includes(req.user.role))return res.status(403).json({error:'Staff or Admin only'});await saveSnapshot(key,value);return res.json({ok:true,state:await sanitizeStateFor(req.user)})}
  if(key==='rcs_class_attendance'){if(req.user.role==='Admin')await saveSnapshot(key,value);else if(req.user.role==='Staff'){const minePerson=(await getSnapshot('rcs_people',[])).find(p=>String(p?.uid||'')===String(req.user.uid||''));const cls=String(minePerson?.className||'');const ex=await getSnapshot(key,[]);const mine=value.filter(a=>String(a.className||'')===cls&&String(a.staffUid||'')===String(req.user.uid||''));await saveSnapshot(key,ex.filter(a=>!(String(a.className||'')===cls&&String(a.staffUid||'')===String(req.user.uid||''))).concat(mine))}else return res.status(403).json({error:'Staff or Admin only'});return res.json({ok:true,state:await sanitizeStateFor(req.user)})}
...
app.post('/api/attendance/sync',auth,async(req,res)=>{try{const incoming=Array.isArray(req.body.value)?req.body.value:[],allowed=(req.user.role==='Admin'||req.user.role==='Activity Supervisor')?incoming:incoming.filter(r=>String(r?.uid||'')===String(req.user.uid||''));await upsertAttendance(allowed);res.json({ok:true,state:await sanitizeStateFor(req.user)})}catch(e){res.status(500).json({error:'Unable to sync attendance'})}});
...
app.post('/api/import',auth,adminOnly,async(req,res)=>{try{const x=req.body||{};for(const k of ['rcs_people','rcs_users','rcs_greports','resurgence_daily_reports','rcs_assignments','rcs_answers','rcs_questions','rcs_parent_comments','rcs_results','rcs_student_reports','rcs_class_attendance'])if(Array.isArray(x[k]))await saveSnapshot(k,x[k]);if(Array.isArray(x.rcs_users))await syncAuthFromUsers(x.rcs_users);if(Array.isArray(x.rcs_att))await upsertAttendance(x.rcs_att);res.json({ok:true,state:await sanitizeStateFor(req.user)})}catch(e){res.status(500).json({error:'Unable to import backup'})}});
app.get('/api/export',auth,adminOnly,async(req,res)=>{try{const users=(await getSnapshot('rcs_users',[])).map(safeUser);const payload={rcs_people:await getSnapshot('rcs_people',[]),rcs_users:users,rcs_greports:await getSnapshot('rcs_greports',[]),resurgence_daily_reports:await getSnapshot('resurgence_daily_reports',[]),rcs_assignments:await getSnapshot('rcs_assignments',[]),rcs_answers:await getSnapshot('rcs_answers',[]),rcs_questions:await getSnapshot('rcs_questions',[]),rcs_parent_comments:await getSnapshot('rcs_parent_comments',[]),rcs_results:await getSnapshot('rcs_results',[]),rcs_student_reports:await getSnapshot('rcs_student_reports',[]),rcs_class_attendance:await getSnapshot('rcs_class_attendance',[]),rcs_admin_teacher_chat:await getSnapshot('rcs_admin_teacher_chat',[]),rcs_admin_parent_chat:await getSnapshot('rcs_admin_parent_chat',[]),rcs_att:await getAllAttendance()};res.setHeader('Content-Type','application/json');res.setHeader('Content-Disposition','attachment; filename="resurgence-city-schools-backup.json"');res.send(JSON.stringify(payload,null,2))}catch(e){res.status(500).json({error:'Unable to export backup'})}});
