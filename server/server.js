...
async function sanitizeStateFor(user){
  ...
  if(isAdmin){
    return {
      ...
      resurgence_daily_reports:await getSnapshot('resurgence_daily_reports',[])
    };
  }
  ...
  const allDaily=await getSnapshot('resurgence_daily_reports',[]);
  const myDaily=(Array.isArray(allDaily)?allDaily:[]).filter(r=>r&&String(r.uid||'')===uid);
  return {rcs_people:mine,rcs_users:(Array.isArray(allUsers)?allUsers:[]).filter(u=>u&&String(u.uid||'')===uid).map(safeUser),rcs_greports:[],rcs_att:myAtt,resurgence_daily_reports:myDaily};
}
...
async function init(){
  ...
  if((await pool.query("SELECT 1 FROM snapshots WHERE key='resurgence_daily_reports'")).rowCount===0) await saveSnapshot('resurgence_daily_reports',[]);
...
// Serve the RCS frontend and static assets such as logo.png
...
app.post('/api/snapshot/:key',auth,async(req,res)=>{
  try{
    ...
    if(key==='resurgence_daily_reports'){
...
app.post('/api/import',auth,adminOnly,async(req,res)=>{
  try{
    ...
    if(Array.isArray(incoming.resurgence_daily_reports))await saveSnapshot('resurgence_daily_reports',incoming.resurgence_daily_reports);
...
app.get('/api/export',auth,adminOnly,async(req,res)=>{
  try{
    ...
    const payload={rcs_people:await getSnapshot('rcs_people',[]),rcs_users:users,rcs_greports:await getSnapshot('rcs_greports',[]),resurgence_daily_reports:await getSnapshot('resurgence_daily_reports',[]),rcs_att:await getAllAttendance()};
    res.setHeader('Content-Type','application/json');res.setHeader('Content-Disposition','attachment; filename="resurgence-city-schools-staff-attendance-backup.json"');res.send(JSON.stringify(payload,null,2));
