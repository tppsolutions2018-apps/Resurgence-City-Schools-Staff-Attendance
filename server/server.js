...
async function sanitizeStateFor(user){
 ...
 if(user.role==='Admin') return {rcs_people:(Array.isArray(people)?people:[]).map(stripPerson),rcs_users:(Array.isArray(allUsers)?allUsers:[]).map(safeUser),rcs_greports:await getSnapshot('rcs_greports',[]),rcs_assignments:assignments,rcs_answers:answers,rcs_att:allAtt,resurgence_daily_reports:daily,rcs_admin_teacher_chat:teacherChat,rcs_admin_parent_chat:parentChat};
